(() => {
  ...
  overlay.innerHTML = `<section class="tk-assistant" role="dialog" aria-modal="true" aria-label="Assistente TK"><header class="tk-assistant-head"><div class="tk-assistant-identity"><div class="tk-assistant-logo">TK</div><div><strong>Assistente TK</strong><small>Pronto ad aiutarti</small></div></div><button class="tk-assistant-close" type="button" aria-label="Chiudi">×</button></header><div class="tk-assistant-body"><div class="tk-dialogue" id="tkDialogue"><div class="tk-message">Ciao! Dimmi quale sezione del portale vuoi aprire oppure scrivilo qui sotto.</div></div><form class="tk-command-row" id="tkCommandForm"><input id="tkCommandInput" type="text" autocomplete="off" placeholder="Es. Fammi vedere i mezzi…" aria-label="Comando per Assistente TK"><button class="tk-mic" id="tkMic" type="button" aria-label="Parla con Assistente TK"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="2" width="6" height="12" rx="3"/><path d="M5 10a7 7 0 0 0 14 0M12 17v5M8 22h8"/></svg></button><button class="tk-send" type="submit" aria-label="Invia comando"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m22 2-7 20-4-9-9-4 20-7Z"/><path d="M22 2 11 13"/></svg></button></form><div class="tk-suggestions"><button class="tk-suggestion" data-command="Apri i mezzi">Mezzi</button><button class="tk-suggestion" data-command="Apri il magazzino">Magazzino</button><button class="tk-suggestion" data-command="Apri il personale">Personale</button><button class="tk-suggestion" data-command="Apri le commesse">Commesse</button><button class="tk-suggestion" data-command="Apri i rifornimenti">Rifornimenti</button></div><p class="tk-voice-note" id="tkVoiceNote">Premi il microfono e autorizza il browser al primo utilizzo. Puoi anche scrivere il comando.</p></div></section>`;
  ...
  const processCommand = async raw => {
    ...
    if (/\b(aiuto|cosa puoi fare|comandi)\b/.test(q)) { const reply='Posso aprire mezzi, magazzino, personale, commesse, rifornimenti, centro costi e gestione social.'; addMessage(reply); speak(reply); return; }
    ...
    if (/\b(scadenz|scade|scadono|scadut|revisione|revisioni|assicurazione|bollo|tagliando)\b/.test(q) && /\b(mezzo|mezzi|veicol|auto|macchin|scadenz|revisione|revisioni)\b/.test(q)) {
      const wait='Controllo subito le scadenze dei mezzi.';addMessage(wait);await speak(wait);
      try { const due=expiringVehicles(await loadVehicles());if(!due.length){const reply='No, al momento non ci sono scadenze previste nei prossimi 30 giorni.';addMessage(reply);speak(reply);return;}const preview=due.slice(0,3).map(vehicleLabel).join('; '),more=due.length>3?` e altri ${due.length-3}`:'';navigateAfterSpeech(`Sì, ci sono ${due.length} mezzi con scadenze da controllare: ${preview}${more}. Apro subito le relative schede.`,'gestione-mezzi.html?tk=deadlines');} catch(error){console.error(error);navigateAfterSpeech('Non riesco a leggere le scadenze in questo momento. Apro comunque la Gestione mezzi per permetterti di controllarle.','gestione-mezzi.html?tk=deadlines');}return;
    }

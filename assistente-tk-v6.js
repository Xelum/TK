(() => {
  ...
  overlay.querySelector('.tk-assistant-head small').textContent='Pronto ad aiutarti · versione 6';
  ...
  const parseAmount = raw => {
    ...
    if(raw&&typeof raw==='object')return parseAmount(raw.value??raw.amount??raw.importo??raw.total??raw.oe??raw.offer??raw.offerta??raw.price??raw.prezzo??'');
    let value=String(raw??'').trim().replace(/\s/g,'').replace(/[^0-9,.-]/g,'');
    if(!value)return 0;
    const comma=value.lastIndexOf(','),dot=value.lastIndexOf('.');
    if(comma>-1&&dot>-1)value=comma>dot?value.replace(/\./g,'').replace(',','.'):value.replace(/,/g,'');
    else if(comma>-1)value=/,\d{1,2}$/.test(value)?value.replace(/\./g,'').replace(',','.'):value.replace(/,/g,'');
    else if((value.match(/\./g)||[]).length>1||/\.\d{3}$/.test(value))value=value.replace(/\./g,'');
    return Number(value)||0;
  };
  const amountOf = c => {
    let raw=pick(c,['amount','importo','importoOE','importoO.E.','offer','offerta','oe','offerAmount','oeAmount','economicOffer','economicAmount','offertaEconomica','importoOfferta','importoOffertaEconomica','valore','value','estimatedValue','totalAmount','totale','price','prezzo']);
    if(raw===''||raw==null){const key=Object.keys(c||{}).find(k=>{const n=keyNorm(k);return['importo','amount','offerta','economic','valore','value','prezzo','price'].some(word=>n.includes(word))||n==='oe';});if(key)raw=c[key];}
  ...
  const helpTopics = {
    ...
    personale:'Puoi chiedermi:\n• Devo fare il programma per domani\n• Devo fare il programma degli operai\n• Apri Personale e squadre\n• Apri il programma giornaliero\n• Mostrami ferie e assenze\n\nDentro Personale e squadre, TK può creare le squadre a voce, copiarle dal programma precedente, salvarle e stamparle.',
  ...
  const processCommand = async raw => {
    ...
    if(q.includes('programma')&&(q.includes('operai')||q.includes('squadr')||q.includes('personale')||q.includes('devo fare')||q.includes('prepara')||q.includes('facciamo')||q.trim()==='programma')){
      navigateAfterSpeech('Certo, apro subito Personale e squadre così possiamo preparare insieme il programma per domani.','gestione-personale.html?tk=program&date=tomorrow');return;
    }
